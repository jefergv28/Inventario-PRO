document.addEventListener("DOMContentLoaded", () => {
  ...
  // Función para agregar eventos de acción a una fila
  function agregarEventosAccion(fila) {
    const modificar = fila.querySelector('.modificar');
    const eliminar = fila.querySelector('.eliminar');
    const favorito = fila.querySelector('.bi-heart, .bi-heart-fill');

    modificar.addEventListener('click', modificarFila);
    eliminar.addEventListener('click', eliminarFila);
    favorito.addEventListener('click', marcarFavorito);
  }

  // Adjuntar eventos iniciales a filas existentes si las hay
  function attachRowEvents() {
    const filas = inventoryTable.querySelectorAll("tr");
    filas.forEach(fila => agregarEventosAccion(fila));
  }
